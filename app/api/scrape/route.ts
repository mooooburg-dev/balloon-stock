...
import { scrapeAllCategories, scrapeByCategory, createMultiCategoryExcelFile, SITES } from '@/lib/scraper';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`사이트: ${siteId}, 크롤링 모드: ${mode}, 선택된 카테고리: ${categories ? categories.join(', ') : '전체'}`);
    ...
    // 카테고리별 다중 탭 엑셀 파일 생성
    const excelBuffer = createMultiCategoryExcelFile(categoryProductsMap, siteId);
...
export async function GET(request: Request) {
  try {
    ...
    console.log(`미리보기 요청 - 사이트: ${siteId}, 카테고리: ${category || '전체'}`);
